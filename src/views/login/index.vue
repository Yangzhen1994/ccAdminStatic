    ...
    <el-form class="login-form" autoComplete="on" :model="loginForm" :rules="loginRules" ref="loginForm" label-position="left">
      ...
      <el-form-item prop="password">
        ...
        <span class="show-pwd" @click="showPwd">
          <svg-icon icon-class="eye" v-if="passwordType"/>
          <i class="el-icon-view" v-if="!passwordType"></i>
        </span>
      </el-form-item>
      <el-button type="primary" style="width:30%;margin-bottom:30px;float: right" :loading="loading" @click.native.prevent="handleLogin('editor')">{{$t('login.logIn')}}</el-button>
...
export default {
  ...
  data() {
    ...
    const validatePassword = (rule, value, callback) => {
      ...
      //.log(this.$sha256(this.loginForm.password))
      if (value.length < 6) {
    ...
    handleLogin(role) {
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          this.loading = true
          const sendData = {}
          sendData.user = this.loginForm.username
          sendData.password = this.loginForm.password //this.$sha256(this.loginForm.password)
          // sendData.role = role
